...
from app.utils.chromadb import get_chromadb_colletion
...
def extract_document_with_mistral(
    ...
    path = Path(file_path)
    base64_document =base64.b64decode(path.read_bytes()).decode("utf-8")

    mistral_client = get_mistral_client()
    return mistral_client.ocr.process(
        model=settings.misral_ocr_model,
        document={
            "type": "document_url",
        ...
        message=[
            {
                "role": "system",
                "content": (
                    "Extract concise key points as bullet points. Return only bullets"
                )
            },
            {
                "role": "user",
...
def insert_page_to_chromadb(
    ...
    collection = get_chromadb_colletion()
    collection.add(
        id=[f"document={document_id}-page-{page_number}"],
        documents=content,
        metadatas=[
            {
